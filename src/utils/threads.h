...
# endif
		...
		namespace detail	{
			TThreadHandle createThreadImpl(void (*func)(void *),void *param);
		...
		/** Returns a handle to the current thread.
		  */
		TThreadHandle getCurrentThreadHandle() ;
		...
		/** Change the priority of the given thread.
		  ...
		  */
		void changeThreadPriority( const TThreadHandle &threadHandle, TThreadPriority priority );

		/** Terminate a thread, giving it no choice to delete objects, etc (use only as a last resource) */
		void terminateThread( TThreadHandle &threadHandle) ;
