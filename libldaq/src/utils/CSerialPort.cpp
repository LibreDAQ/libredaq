...
void CSerialPort::open()
{
    ...
    // "controlling terminal" for that port. The O_NDELAY flag tells UNIX that
    // this program doesn't care what state the DCD signal line is in - whether
    // the other end of the port is up and running.
    if (-1 ==
        (hCOM = ::open(m_serialName.c_str(), O_RDWR | O_NOCTTY | O_NDELAY)))
        throw std::runtime_error(libredaq::format(
    ...
    if (tcflush(hCOM, TCIFLUSH) < 0)
        throw std::runtime_error(
            libredaq::format("Cannot flush serial port: %s", strerror(errno)));
...
isOpen
----------------------------------------------------- */
bool CSerialPort::isOpen() const { return hCOM != -1; }
    ...
    int baudRate, int parity, int bits, int nStopBits, bool enableFlowControl)
{
    ...
    if (baudRate <= 0)
        throw std::runtime_error("Serial port baudRate must be >0!");
    ...
    if ((cfsetispeed(&port_settings, BR) < 0) ||
        ...
        throw std::runtime_error(libredaq::format(
            "Cannot change baudRate in setting structure: %s",
            strerror(errno)));
    ...
    switch (bits)
    {
        ...
        default:
            throw std::runtime_error(
                libredaq::format("Invalid character size: %i", bits));
            break;
    }
    ...
    switch (parity)
    {
        ...
        default:
            throw std::runtime_error(
                libredaq::format("Invalid parity selection: %i", parity));
            break;
    }
    ...
    switch (nStopBits)
    {
        ...
        default:
            throw std::runtime_error(
                libredaq::format("Invalid number of stop bits: %i", nStopBits));
            break;
    }
    ...
    if (tcsetattr(hCOM, TCSANOW, &port_settings) < 0)
        throw std::runtime_error(libredaq::format(
            "Cannot set the new settings: %s", strerror(errno)));
...
void CSerialPort::close()
{
    ...
    //    PosixSignalDispatcher::Instance() ; signal_dispatcher.DetachHandler(
    //    SIGIO, *this ) ;
 ...
 *  This method reads from the serial port until one of the characters in \a eol
 * are found.
 */
    ...
    while (total_timeout_ms < 0 || (m_timer.Tac() * 1e3 < total_timeout_ms))
    {
        ...
        if (ioctl(hCOM, FIONREAD, &waiting_bytes) < 0)
        {
            ...
            {  // The port has been disconnect (for USB ports)
                this->close();
                throw std::runtime_error(
                    "Error reading port before end of line");
            }
        }
...
isOpen
----------------------------------------------------- */
bool CSerialPort::isOpen() const { return hCOM != NULL; }
    ...
    int baudRate, int parity, int bits, int nStopBits, bool enableFlowControl)
{
    ...
    switch (baudRate)
    {
        ...
        default:
            BR = baudRate;
            // throw std::runtime_error(libredaq::format("Invalid desired baud
            // rate value: %i",baudRate ) );
            break;
    }
    ...
    int WriteTotalTimeoutConstant)
{
    ...
    // Port must be open!
    if (!isOpen()) throw std::runtime_error("The COM port is not open");
    ...
    const int total_timeout_ms, bool* out_timeout, const char* eol_chars)
{
    ...
    if (eol_chars == NULL)
        throw std::runtime_error("eol_chars cannot be NULL!");
...
size_t CSerialPort::Write(const void* Buffer, size_t Count)
{
    ...
    if (!WriteFile(hCOM, Buffer, (DWORD)Count, &actuallyWritten, NULL))
...
{
    if (isOpen())
        throw std::runtime_error("Cannot change serial port while open");
