 ...
 * timeouts,...), most of the methods will raise an exception of the class
 * "std::exception"
 *
...
class CSerialPort
{
   public:
     ...
     * this page. \param openNow Whether to try to open the port now. If not
     * selected, the port should be open later with "open()".
     */
     ...
     * match, an exception is raised. \exception std::exception On communication
     * errors or a different serial port already open.
     */
     ...
     * (RTS/CTS) (default=no) \exception std::exception On communication errors
     */
    void setConfig(
    ...
    /** Read incoming data. This method will not raise an exception on zero
     * bytes read, as long as there is not any fatal error in the
     * communications. \exception std::exception On communication errors
     */
     ...
     * eol are found. \param eol_chars A line reception is finished when one of
     * these characters is found. Default: LF (10), CR (13). \param
     ...
     * ocurred, false on a valid read. \return The read string, without the
     * final \exception std::exception On communication errors
     */
    std::string ReadString(
        const int total_timeout_ms = -1, bool* out_timeout = NULL,
     ...
     * number of bytes actually written. \exception std::exception On
     * communication errors
     */
    size_t Write(const void* Buffer, size_t Count);

   protected:
    std::string m_serialName;  //!< The complete name of the serial port device
                               //!< (i.e. "\\.\COM10","/dev/ttyS2",...)
    int     m_baudRate;
