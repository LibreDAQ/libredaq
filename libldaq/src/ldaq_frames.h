...
/** The numerical values of each OPCODE identifier. See the list of frames below
 * to learn on each OPCODE meaning */
 ...
 * - Firmware: Add new firmware modes here, together with the proper call to
 * ldaq_register_firmware_mode()
 */
typedef enum
{
    FIRM_MODE_NORMAL = 0,
...
#define DECLARE_LDAQ_FRAME_END(_STRUCT_NAME_) \
    uint8_t tail;                             \
    _STRUCT_NAME_(const uint8_t opcode_)      \
        : header(LDAQ_FRAME_START),           \
          opcode(opcode_),                    \
          len(sizeof(_STRUCT_NAME_) - 4),     \
          tail(LDAQ_FRAME_END)                \
    {                                         \
    }                                         \
    }                                         \
...
#define DECLARE_LDAQ_FRAME_END(_STRUCT_NAME_)          \
    ...
    ;                                                  \
    static inline void _STRUCT_NAME_##_init(           \
        struct _STRUCT_NAME_* f, const uint8_t opcode) \
    {                                                  \
...
uint8_t  src_slot;  //!< Source of this measurements: Slot index
                   //!< (0=first,1=second,...) or 0xff means microcontroller
                   //!< embedded sensors.
...
DECLARE_LDAQ_FRAME_END_OPCODE(
    TFrame_LDAQCMD_ADC_AMP_Start, FRAMECMD_ADC_AMP_START)

DECLARE_LDAQ_FRAME_BEGIN(TFrame_LDAQCMD_ADC_Stop_AMP)
uint8_t slot;  //!< Which slot to head this command to
DECLARE_LDAQ_FRAME_END_OPCODE(
    TFrame_LDAQCMD_ADC_Stop_AMP, FRAMECMD_ADC_AMP_STOP)
...
DECLARE_LDAQ_FRAME_END_OPCODE(
    TFrame_LDAQCMD_DAC_SetValues, FRAMECMD_DAC_SET_VALUES)

DECLARE_LDAQ_FRAME_BEGIN(TFrame_LDAQCMD_SwitchFirmwareMode)
uint8_t new_firmware_mode;
DECLARE_LDAQ_FRAME_END_OPCODE(
    TFrame_LDAQCMD_SwitchFirmwareMode, FRAMECMD_FIRMWARE_MODE)
